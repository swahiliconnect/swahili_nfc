...
class SessionManager {
  ...
  Future<void> deactivateCard({required String cardId}) async {
    try {
      ...
      final updatedCards =
          cards.where((card) => card.cardId != cardId).toList();
  ...
  void enableOfflineMode({
    ...
    if (_offlineCardCache.length > _maxCacheSize) {
      ...
      final sortedKeys = _offlineCardCache.keys.toList()
        ..sort((a, b) => _offlineCardCache[a]!
            .createdAt
            .compareTo(_offlineCardCache[b]!.createdAt));
  ...
  String generateCardUrl({
    ...
    if (_baseUrl == null) {
      throw NFCError(
        ...
        message:
            'SwahiliCard not configured. Call configureForSwahiliCard first.',
      );
    }
